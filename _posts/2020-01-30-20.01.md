...
Happy with a place to put his talents to work, and with large goals in mind, **Em3rgency** added a new [Centcom repository]({{ site.github_url }}/CentCom){:target="_blank"} to our organization. This acts as a central server for managing user registration, authentication, and character storage, similar to what Byond is to SS13.
...
If you don't want to download Unity and build the most recent development changes yourself, you can instead [**download our latest build (v0.3.0) here**]({{ site.github_url }}/SS3D/releases/latest){:target="_blank"}, which has been updated recently. And we also hope to update it frequently by integrating automated builds in the future for each minor release.
